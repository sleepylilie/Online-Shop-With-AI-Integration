...
// =============
const breadcrumbs = document.getElementById('catalog__breadcrumbs');
const titleEl = document.getElementById('catalog__title');
const subcategoryGrid = document.getElementById('catalog__subcategory-grid');
const productList = document.getElementById('catalog__product-list');
const productCount = document.getElementById('catalog__product-count');
const sortSelector = document.getElementById('catalog__sort-selector');
const filterToggleBtn = document.getElementById('filters__toggle');
const filterPanel = document.getElementById('catalog__filter-panel');
const filterCloseBtn = document.getElementById('filters__close-panel');
const filterApplyBtn = document.getElementById('filters__apply');
const filterResetBtn = document.getElementById('filters__reset');
const filterSubcategoryList = document.getElementById('filter__subcategory-list');
const filterSubcategoryGroup = document.getElementById('filter-group__subcategory');
const priceFromInput = document.getElementById('filters__price-from');
const priceToInput = document.getElementById('filters__price-to');
const inStockCheckbox = document.getElementById('filter__in-stock');
const highRatingCheckbox = document.getElementById('filter__high-rating');
// =============
// Состояние страницы
// =============
...
const renderSubcategoryGrid = () => {
    ...
    allSubcategories.forEach(sub => {
        ...
        card.addEventListener('click', (e) => {
            ...
            subcategoryGrid.querySelectorAll('.category-card').forEach(c => c.classList.toggle('active', c.getAttribute('data-subcategory-url') === activeSubUrlKey));
...
const renderSubcategoryCheckboxes = () => {
    ...
    allSubcategories.forEach(sub => {
        ...
        input.addEventListener('change', () => {
            if (input.checked) {
                filterSubcategoryList.querySelectorAll('input[type="checkbox"]')
                    .forEach(cb => { if (cb !== input)
                    cb.checked = false; });
            }
        });
...
const syncSubcategoryCheckboxes = () => {
    filterSubcategoryList.querySelectorAll('input[type="checkbox"]')
...
const loadProducts = async () => {
    ...
    if (subKey) {
        ...
        if (sub)
            params.subcategory_id = String(sub.id);
    }
    if (searchQuery)
    ...
    if (appliedFilters.priceFrom)
        params.price_from = appliedFilters.priceFrom;
    if (appliedFilters.priceTo)
        params.price_to = appliedFilters.priceTo;
    if (appliedFilters.inStock)
        params.in_stock = 'true';
    if (appliedFilters.highRating)
        params.min_rating = '4';
    params.limit = '40';
    try {
...
if (priceFromInput)
    constrainPriceInput(priceFromInput);
if (priceToInput)
    constrainPriceInput(priceToInput);
// Кнопка "Применить фильтры"
filterApplyBtn.addEventListener('click', () => {
    // Считываем выбранную подкатегорию из чекбоксов
    const checkedSub = filterSubcategoryList.querySelector('input:checked');
...
filterResetBtn.addEventListener('click', () => {
    filterSubcategoryList.querySelectorAll('input').forEach(cb => cb.checked = false);
