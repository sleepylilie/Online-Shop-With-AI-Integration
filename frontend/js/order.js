...
const fmt = (n) => n.toLocaleString('ru-RU', { minimumFractionDigits: 0 }) + ' ₽';
...
const initDeliveryButtons = () => {
    ...
    const dateButtons = datesContainer.querySelectorAll('.order__delivery-time');
    ...
    const timeButtons = timesContainer.querySelectorAll('.order__delivery-time');
...
// =============
const initProfileCheckbox = () => {
    const checkbox = document.getElementById('order__use-profile-info');
    if (!checkbox)
        return;
...
const checkFormValidity = () => {
    const submitBtn = document.getElementById('order__submit');
    if (!submitBtn)
        return;
    const get = (id) => document.getElementById(id)?.value.trim() ?? '';
    // Форма валидна только если все обязательные поля заполнены правильно
    const valid = get('order__last-name') !== '' &&
...
const initFieldValidation = () => {
    ...
    fields.forEach(([inputId, msgId, validator]) => {
        if (inputId === 'order__phone')
            return; // телефон обрабатывается отдельно ниже
...
const submitOrder = async () => {
    ...
    const get = (id) => document.getElementById(id)?.value.trim() ?? '';
    const getRadio = () => (document.querySelector('input[name="payment_method"]:checked'))?.value ?? '';
    const payload = {
