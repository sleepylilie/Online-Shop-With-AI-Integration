...
const formatRating = (rating) => rating && parseFloat(rating) > 0
...
// Элементы страницы
// =============
const breadcrumbsEl = document.getElementById('product__breadcrumbs');
const titleEl = document.getElementById('product__title');
const mainImageEl = document.getElementById('product__main-image');
const thumbnailsEl = document.getElementById('product__thumbnails');
const prevBtn = document.getElementById('product__prev-image');
const nextBtn = document.getElementById('product__next-image');
const tagsContainer = document.getElementById('product__tags');
const tagNew = document.getElementById('product__tag-new');
const tagSale = document.getElementById('product__tag-sale');
const descShortEl = document.getElementById('product__desc-short');
const ratingShortEl = document.getElementById('product__rating-short');
const attrsShortEl = document.getElementById('product__attrs-short');
const goToReviewsLink = document.getElementById('product__go-to-reviews');
const goToAttrsLink = document.getElementById('product__go-to-attrs');
const oldPriceEl = document.getElementById('product__old-price');
const saleTagEl = document.getElementById('product__sale-tag');
const salePercentEl = document.getElementById('product__sale-percent');
const priceEl = document.getElementById('product__price');
const stockEl = document.getElementById('product__stock');
const btnCart = document.getElementById('product__btn-cart');
const btnFavorite = document.getElementById('product__btn-favorite');
const iconFavorite = btnFavorite?.querySelector('.product__icon-favorite');
const btnShare = document.getElementById('product__btn-share');
const relatedList = document.getElementById('product__related-list');
const tabButtons = document.querySelectorAll('.about-product__controller-button');
const descFullEl = document.getElementById('product__desc-full');
const attrsFullEl = document.getElementById('product__attrs-full');
const reviewsListEl = document.getElementById('product__reviews-list');
const ratingTotalEl = document.getElementById('product__rating-total');
const reviewsCountEl = document.getElementById('product__reviews-count');
// =============
// 1. Хлебные крошки
// =============
...
const setMainImage = (index) => {
    ...
    thumbnailsEl.querySelectorAll('.product-thumbnail').forEach((t, i) => t.classList.toggle('active', i === currentImageIndex));
...
const renderAttributes = (container, attrs, limit = 0) => {
...
const loadReviews = async (productId) => {
    ...
    try {
        ...
        if (ratingShortRef)
            ratingShortRef.textContent = ratingFormatted;
        // Шаблон для карточек отзывов
        reviews.forEach(r => {
            // Звездочки рейтинга
            const stars = Array.from({ length: 5 }, (_, i) => `<img src="/static/assets/icons/icon_star-filled_${i < r.rating ? 'yellow' : 'gray'}.svg" style="width:16px;height:16px" alt="" />`).join('');
            // Путь к аватару пользователей
            const avatarSrc = fixMediaUrl(r.user_avatar) ?? '/media/profile-avatars/avatar-placeholder.png';
...
const toggleFavorite = async (productId) => {
    ...
    try {
        if (isFav) {
            ...
            if (iconFavorite)
                iconFavorite.src = '/static/assets/icons/icon_liked.svg';
        }
        else {
...
const setupScrollArrows = (list) => {
    ...
    const step = () => { const c = list.querySelector('.product-card'); return c ? c.offsetWidth + 20 : 300; };
