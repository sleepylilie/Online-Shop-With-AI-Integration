...
// Поля ввода и кнопка
const newPwdInput = document.getElementById('change-password__new') as HTMLInputElement;
const confirmInput = document.getElementById('change-password__confirm') as HTMLInputElement;
const submitBtn = document.getElementById('change-password__submit') as HTMLButtonElement;
...
const showErr = (el: HTMLElement, text: string): void => {
    el.textContent = text;
...
submitBtn?.addEventListener('click', async () => {
    ...
    let hasErr = false;
    if (!newPwd || newPwd.length < 6) { showErr(newMsg, 'Пароль должен быть не менее 6 символов.'); hasErr = true; }
    if (!confirm)                     { showErr(confirmMsg, 'Повторите пароль.'); hasErr = true; }
    else if (newPwd !== confirm)      { showErr(confirmMsg, 'Пароли не совпадают.'); hasErr = true; }
    if (hasErr) return;
