...
/**
 * product-card.ts - карточка товара
 *
 * Этот файл используется везде, где показываются товары:
 * - на главной, 
 * - в каталоге, 
 * - в поиске, 
 * - на странице товара.
...
const calcDiscount = (price: string, oldPrice: string): string | null => {
...
const formatRating = (rating: string | null): string =>
    rating ? parseFloat(rating).toFixed(2).replace('.', ',') : '—';
...
const addToCart = async (productId: number, btn: HTMLButtonElement): Promise<void> => {
...
const toggleFavorite = async (productId: number, btn: HTMLButtonElement): Promise<void> => {
    // Проверяем авторизацию
    const token = getAuthToken();
    if (!token) { alert('Войдите в аккаунт, чтобы добавить товар в избранное.'); return; }

    // Смотрим текущее состояние по классу кнопки
    const isFavorite = btn.classList.contains('is-favorite');
    const iconImg = btn.querySelector<HTMLImageElement>('.product-card__icon-favorite');
    ...
    try {
        if (isFavorite) {
            ...
            if (iconImg) iconImg.src = ICON.liked;
        } else {
            ...
            if (iconImg) iconImg.src = ICON.likedFilled;
        }
    } catch {
...
let _cartProductIds: Set<number> | null = null;
let _favoriteProductIds: Set<number> | null = null;
let _cacheLoading = false;
const _cacheCallbacks: (() => void)[] = [];

const loadStatusCache = async (): Promise<void> => {
    // Если кэш уже загружен, ничего не делаем
    if (_cartProductIds !== null) return;

    // Если загрузка уже идет, ожидаем
    if (_cacheLoading) {
        await new Promise<void>(resolve => _cacheCallbacks.push(resolve));
        return;
    }
    _cacheLoading = true;
    ...
    try {
        ...
        const cartItems: { product_id: number }[] = cartRes.ok ? await cartRes.json() : [];
        const favItems:  { product_id: number }[] = favRes.ok  ? await favRes.json()  : [];
...
export const invalidateStatusCache = (): void => {
...
export const createProductCard = (product: ProductPreview, _preloaded = false): HTMLElement => {
    ...
    const btnCart = card.querySelector<HTMLButtonElement>('.product-card__btn-cart')!;
    const btnFav  = card.querySelector<HTMLButtonElement>('.product-card__btn-favorite')!;
    ...
    btnCart.addEventListener('click', (e) => {
        ...
        if (_cartProductIds) _cartProductIds.add(product.id);
    });
    ...
    loadStatusCache().then(() => {
        ...
        if (_favoriteProductIds?.has(product.id) && btnFav) {
            const icon = btnFav.querySelector<HTMLImageElement>('.product-card__icon-favorite');
            btnFav.classList.add('is-favorite');
            btnFav.title = 'Убрать из избранного';
            if (icon) icon.src = ICON.likedFilled;
        }
    });
