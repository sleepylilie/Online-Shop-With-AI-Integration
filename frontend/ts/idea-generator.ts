...
// Структура одного сообщения в истории
...
const generateBtn = document.getElementById('chat__generate-btn') as HTMLButtonElement;
...
const addBotMessage = (html: string, isLoading = false): HTMLElement => {
...
const sendGeneration = async (): Promise<void> => {
...
const loadHistoryFromDB = async (): Promise<boolean> => {
    if (!tok()) return false;
    ...
    try {
        ...
        recent.forEach((m: Record<string, string>) => {
...
const restoreFromSession = (): boolean => {
    ...
    if (lastIsAssistant) addAfterGenButtons();
    else generateForm.style.display = 'flex';
...
// Описание структуры шаблона идеи из таблицы idea_templates
...
const loadIdeas = async (): Promise<void> => {
    ...
    try {
        ...
        const ideas: IdeaTemplate[] = await res.json();
