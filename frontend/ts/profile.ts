...
const isDesktop = (): boolean => window.innerWidth >= 1280;
...
const showTab = (tabName: string): void => {
    ...
    document.querySelectorAll<HTMLElement>('.profile__tab').forEach(t => {
    ...
    document.querySelectorAll<HTMLButtonElement>('.profile__nav-option').forEach(btn => {
...
document.querySelectorAll<HTMLElement>('.profile__nav-option').forEach(btn => {
    // Элемент с именем пользователя пропускается (не кликабельный)
    if (btn.classList.contains('profile__nav-option--user')) return;
...
document.querySelectorAll<HTMLButtonElement>('.profile__back-btn').forEach(btn => {
...
const loadTabData = (tab: string): void => {
...
const loadPersonalInfo = async (): Promise<void> => {
    ...
    try {
        ...
        if (user.gender) {
            const radio = document.querySelector<HTMLInputElement>(`input[name="profile-gender"][value="${user.gender}"]`);
            if (radio) radio.checked = true;
        }

        // Подставляется нужный аватар пользователя
        const avatarImg = document.getElementById('profile__avatar-img') as HTMLImageElement | null;
        ...
        if (navName) navName.textContent = user.first_name || 'Профиль';
...
const applyPhoneMaskProfile = (raw: string): string => {
    ...
    else if (digits.length === 3) r += ')';
    if (digits.length > 6)  r += '-' + digits.slice(6, Math.min(8, digits.length));
    else if (digits.length === 6) r += '-';
    if (digits.length > 8)  r += '-' + digits.slice(8, 10);
    else if (digits.length === 8) r += '-';
    return r;
};

// Проверка, что телефон введен полностью
const isPhoneComplete = (masked: string): boolean => {
    return masked.replace(/\D/g, '').length === 11;
};

const phoneInput = document.getElementById('profile__phone') as HTMLInputElement | null;
...
const hasFormChanged = (): boolean => {
    ...
    const gender = (document.querySelector<HTMLInputElement>('input[name="profile-gender"]:checked'))?.value ?? '';
    return (
        get('profile__first-name') !== (currentUser['first_name'] ?? '') ||
...
const updateSaveButtonState = (): void => {
    const btn = document.getElementById('profile__save-btn') as HTMLButtonElement | null;
    if (!btn) return;
...
document.querySelectorAll<HTMLInputElement>('input[name="profile-gender"]').forEach(r => {
...
document.getElementById('profile__save-btn')?.addEventListener('click', async () => {
    ...
    const firstName = (document.getElementById('profile__first-name') as HTMLInputElement).value.trim();
    const lastName = (document.getElementById('profile__last-name') as HTMLInputElement).value.trim();
    const email = (document.getElementById('profile__email') as HTMLInputElement).value.trim();
    const phone = (document.getElementById('profile__phone') as HTMLInputElement).value.trim();
    const gender = (document.querySelector<HTMLInputElement>('input[name="profile-gender"]:checked'))?.value ?? '';
    ...
    try {
        ...
        if (res.ok) {
            ...
            const headerSpan = document.querySelector<HTMLElement>('#header__btn-profile span');
            if (headerSpan) headerSpan.textContent = firstName;
            const navName = document.getElementById('profile__nav-username');
    ...
    } catch { saveMsg.textContent = 'Ошибка сети.'; saveMsg.style.display = 'block'; }
});
...
document.getElementById('profile__avatar-input')?.addEventListener('change', async (e) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const img = document.getElementById('profile__avatar-img') as HTMLImageElement | null;
            if (img) img.src = data.avatar + '?v=' + Date.now();
            avatarMsg.style.display = 'none';
        } else {
            avatarMsg.textContent   = data.error || 'Ошибка загрузки.';
            avatarMsg.style.display = 'block';
        }
    } catch { avatarMsg.textContent = 'Ошибка сети.'; avatarMsg.style.display = 'block'; }
});
...
document.getElementById('profile__confirm-pwd-btn')?.addEventListener('click', async () => {
    const input = document.getElementById('profile__current-pwd') as HTMLInputElement;
    const msg = document.getElementById('profile__pwd-msg')!;
    const pwd = input?.value ?? '';

    if (!pwd) { msg.textContent = 'Введите пароль.'; msg.style.display = 'block'; return; }
    ...
    } catch { msg.textContent = 'Ошибка сети.'; msg.style.display = 'block'; }
});
...
document.getElementById('profile__delete-account-btn')?.addEventListener('click', async () => {
    ...
    if (!confirm('Вы уверены что хотите удалить аккаунт?\nЭто действие необратимо, все ваши данные будут удалены.')) return;
    ...
    try {
        ...
        if (!checkData.valid) { alert('Неверный пароль. Удаление отменено.'); return; }
...
const loadNotifications = async (type: string): Promise<void> => {
    notifType = type;
    ...
    try {
        ...
        data.forEach((n: Record<string, string | boolean>) => {
    ...
    } catch { list.innerHTML = '<p class="text__body-smaller" style="color:var(--error)">Ошибка загрузки.</p>'; }
};

// Кнопки фильтра уведомлений переключает активность фильтра
document.querySelectorAll<HTMLButtonElement>('[data-notif-type]').forEach(btn => {
    btn.addEventListener('click', () => {
...
const loadOrders = async (filter: string): Promise<void> => {
    ...
    try {
        ...
        data.forEach((order: Record<string, unknown>) => {
            ...
            const itemImgs = items.map((item: Record<string, unknown>) =>
                `<div class="order-item" style="cursor:pointer" data-url-key="${item['url_key'] || ''}">
            ...
            const rateBtn = card.querySelector<HTMLButtonElement>('.rate-order-btn');
            if (rateBtn) {
    ...
    } catch { list.innerHTML = '<p class="text__body-smaller" style="color:var(--error)">Ошибка загрузки заказов.</p>'; }
};

// Кнопки фильтра заказов
document.querySelectorAll<HTMLButtonElement>('[data-order-filter]').forEach(btn => {
    btn.addEventListener('click', () => {
...
const loadFavorites = async (): Promise<void> => {
    ...
    try {
        ...
        data.forEach((item: Record<string, unknown>) => {
            ...
            const favBtn  = card.querySelector<HTMLButtonElement>('.product-card__btn-favorite');
            const favIcon = card.querySelector<HTMLImageElement>('.product-card__icon-favorite');
            if (favBtn && favIcon) {
    ...
    } catch { grid.innerHTML = '<p class="text__body-smaller" style="color:var(--error)">Ошибка загрузки.</p>'; }
};
...
const loadReviews = async (filter: string): Promise<void> => {
    ...
    try {
        ...
        data.forEach((r: Record<string, unknown>) => {
            ...
            const stars = Array.from({ length: 5 }, (_, i) =>
                `<img src="/static/assets/icons/icon_star-filled_${i < Number(r['rating']) ? 'yellow' : 'gray'}.svg"
...
// Фильтры отзывов
document.querySelectorAll<HTMLButtonElement>('[data-review-filter]').forEach(btn => {
    btn.addEventListener('click', () => {
...
const updateStars = (rating: number): void => {
    document.querySelectorAll<HTMLImageElement>('.review-star').forEach(star => {
...
const saveReviewedToStorage = (set: Set<number>): void => {
    localStorage.setItem(REVIEWED_KEY, JSON.stringify(Array.from(set)));
};

// Загружаем из localStorage при старте
const loadReviewedFromStorage = (): Set<number> => {
    try {
...
// Синхрон с сервером
const syncReviewedFromAPI = async (): Promise<void> => {
    if (!tok()) return;
    try {
...
const openNextReview = (): void => {
    ...
    const comment = document.getElementById('review-popup__comment') as HTMLTextAreaElement;
...
document.getElementById('review-popup__submit')?.addEventListener('click', async () => {
    const msg = document.getElementById('review-popup__msg')!;
    const comment = (document.getElementById('review-popup__comment') as HTMLTextAreaElement).value.trim();

    // Обязательно выбрать рейтинг перед отправкой
    if (reviewRating === 0) {
        msg.textContent = 'Выберите оценку.'; msg.style.display = 'block'; return;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            tabLoaded['reviews'] = false;
            if (activeTab === 'reviews') loadReviews('all');

            // Блокировка кнопки оценки, если все товары оценены
            if (pendingReviewIndex >= pendingReviewItems.length) {
                document.querySelectorAll<HTMLButtonElement>('.rate-order-btn').forEach(btn => {
...
document.getElementById('review-popup-overlay')?.addEventListener('click', (e) => {
    if (e.target === document.getElementById('review-popup-overlay')) {
        (document.getElementById('review-popup-overlay') as HTMLElement).style.display = 'none';
    }
});
