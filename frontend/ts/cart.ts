 ...
 *
 * Основные функции:
 *  - товары загружаются из API и отображаются в виде карточек;
 *  - пользователь может выбирать товары чекбоксами и видеть сумму только по ним;
 *  - кнопки + и − меняют количество, но не опускают ниже 1 и не поднимают выше 999;
 *  - кнопка мусорного ведра удаляет один товар, «Удалить выбранные» удаляет сразу несколько;
 *  - при переходе к оформлению ID выбранных товаров передаются в URL следующей страницы.
 */


// =============
// Структура данных товара в корзине
// =============

interface CartItem {
    cart_item_id: number;   // ID записи в корзине 
...
const getSelectAll = () => document.getElementById('cart__select-all') as HTMLInputElement;
...
const recalcSummary = (): void => {
    ...
    cartItems.forEach(item => {
        if (!selectedIds.has(item.cart_item_id)) return;
    ...
    const proceedBtn = document.getElementById('cart__proceed-to-order') as HTMLButtonElement | null;
    if (proceedBtn) {
...
const createCartItemEl = (item: CartItem): HTMLElement => {
    ...
    const checkbox = el.querySelector<HTMLInputElement>('.cart-item__checkbox')!;
    checkbox.addEventListener('change', () => {
        if (checkbox.checked) selectedIds.add(item.cart_item_id);
        else                  selectedIds.delete(item.cart_item_id);
        syncSelectAll();
        recalcSummary();
    });
    ...
    const minusBtn = el.querySelector<HTMLButtonElement>('.quantity__button-minus');
    const plusBtn = el.querySelector<HTMLButtonElement>('.quantity__button-plus');

    const updateBtnStates = (): void => {
        if (minusBtn) {
...
const updateQuantity = async (
    ...
    try {
        ...
        const qEl = el.querySelector<HTMLElement>('.quantity__value');
        if (qEl) qEl.textContent = String(newQty);
        recalcSummary();
...
const toggleFavorite = async (productId: number, el: HTMLElement): Promise<void> => {
    if (!token()) { alert('Сессия истекла. Войдите в аккаунт.'); return; }
    const btn = el.querySelector<HTMLButtonElement>('.cart-item__favorite')!;
    const iconImg = btn.querySelector<HTMLImageElement>('.cart-item__icon-favorite')!;
...
const loadCart = async (): Promise<void> => {
    ...
    try {
        ...
        cartItems = await res.json() as CartItem[];
        container.innerHTML = '';
        ...
        if (authTok) {
            fetch('/api/favorites/', { headers: { 'Authorization': `Bearer ${authTok}` } })
                ...
                .then((favs: { product_id: number }[]) => {
                    ...
                    const favSet = new Set(favs.map((f: { product_id: number }) => f.product_id));
                    container.querySelectorAll<HTMLElement>('.cart-item').forEach(el => {
                        const pid = parseInt(el.getAttribute('data-product-id') ?? '0');
                        if (favSet.has(pid)) {
                            const btn  = el.querySelector<HTMLButtonElement>('.cart-item__favorite');
                            const icon = el.querySelector<HTMLImageElement>('.cart-item__icon-favorite');
                            if (btn && icon) {
                                btn.classList.add('is-favorite');
...
document.getElementById('cart__select-all')?.addEventListener('change', (e) => {
    ...
    selectedIds.clear();
    if (checked) cartItems.forEach(item => selectedIds.add(item.cart_item_id));
    document.querySelectorAll<HTMLInputElement>('.cart-item__checkbox').forEach(cb => {
        cb.checked = checked;
    });
    recalcSummary();
});
